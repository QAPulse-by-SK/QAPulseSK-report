...
function buildPrompt(test: TestResult): string {
...
async function callAnthropic(prompt: string, apiKey: string, model: string): Promise<AIAnalysis | null> {
  try {
...
async function callOpenAI(prompt: string, apiKey: string, model: string): Promise<AIAnalysis | null> {
  try {
...
async function callGemini(prompt: string, apiKey: string, model: string): Promise<AIAnalysis | null> {
  try {
...
export async function analyzeFailures(
  ...
  if (!config.enabled || !config.apiKey) return results;
