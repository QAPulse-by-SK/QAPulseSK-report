...
exports.drainScreenshots = drainScreenshots;
exports._clearScreenshotRegistry = _clearScreenshotRegistry;
const registry = new Map();
function normalize(testName) {
    return testName.trim().replace(/\s+/g, ' ');
}
function attachScreenshot(testFullName, sourcePath, opts = {}) {
    if (!testFullName || !sourcePath)
        return;
...
function drainScreenshots(testFullName) {
    const key = normalize(testFullName);
    const list = registry.get(key) || [];
    registry.delete(key);
    return list;
}
function _clearScreenshotRegistry() {
    registry.clear();
}
//# sourceMappingURL=screenshot-registry.js.map