...
exports.detectMetadata = detectMetadata;
...
function safeGit(cmd) {
    try {
        return (0, child_process_1.execSync)(`git ${cmd}`, { stdio: ['ignore', 'pipe', 'ignore'], timeout: 1500 })
...
function detectCi() {
    ...
    if (process.env.GITHUB_ACTIONS) {
        ...
        if (repo && ci.jobId)
            ci.jobUrl = `https://github.com/${repo}/actions/runs/${ci.jobId}`;
        const ref = envAny('GITHUB_REF') || '';
        const prMatch = ref.match(/refs\/pull\/(\d+)\//);
        if (prMatch) {
            ci.prNumber = prMatch[1];
            if (repo)
                ci.prUrl = `https://github.com/${repo}/pull/${prMatch[1]}`;
        }
        return ci;
    }
    // GitLab CI
...
function detectGit() {
    ...
    if (git.commit)
        git.commitShort = git.commit.slice(0, 7);
    git.commitMessage =
...
function detectMetadata() {
    ...
    const git = detectGit();
    const ci = detectCi();
    if (Object.keys(git).some(k => git[k]))
        meta.git = git;
...
function mergeMetadata(existing, detected) {
    return {
        ...detected,
        ...(existing || {}),
